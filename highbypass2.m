function [Fovermdot, S,h_0, f0, eta_P, eta_TH, rateofV9toa0, rateofV19toa0, Pt9overP9, Pt19overP19, P19overP0, P9overP0, T9overT0, T19overT0, pi_tH, pi_tL, tau_f,tau_r, tau_cL, tau_cH, tau_tH, tau_tL, tau_lambda,tau_m1,tau_m2, f, eta_f, eta_cL, eta_cH, eta_tH, eta_tL, M_9, M_19,A_4,A_4_5,A_9,A_19,pi_r,pi_d,pi_cL,pi_cH,MFP_4] = highbypass(M_0, T_0, P_0, beta, C_TOL, C_TOH, h_PR, epsilon1, epsilon2, pi_b, pi_dmax, pi_n, pi_nf, e_f, e_cL, e_cH, e_tH, e_tL, eta_b, eta_mL, eta_mH, eta_mPL, eta_mPH, pi_f, pi_cL, pi_cH, alfa, Tt4,mdot_0)
                    % Constants
    g = 9.81;
    f = 0;
    ...
    while abs(f - f4i) > 0.00001
        ...
        f = (h_t4 - h_t3) / (eta_b * h_PR - h_t4);
            ...
            %Burner area4 calculation

    [M_4, T_total4_over_T4, P_total4_over_P4, MFP_4] = RGCOMPR(1, Tt4, f, 0.8);
    tau_lambda=h_t4/h_0;
                %coolant mixer
    ...
    [M_4, T_total4_over_T4, P_total4_over_P4, MFP_4] = RGCOMPR(1, T_t4_5, f_4_5, 1);
    ...
    mdot_4=mdot_0/(1+alfa)*(1-beta-epsilon1-epsilon2)*(1+f);
    A_4=mdot_4*sqrt(Tt4)/((P_0*pi_r*pi_d*pi_cL*pi_cH*pi_b)*MFP_4);
            %A19 calculation

    mdot_f=(alfa)*mdot_0/(alfa+1); %fan mass flow rate
    A_19=mdot_f*sqrt(T_t13)/((P_0*pi_r*pi_d*pi_f*pi_nf)*MFP_19);
            %A4.5 calculation

    [M_4_5, T_total4_5_over_T4_5, P_total4_5_over_P4_5, MFP_4_5] = RGCOMPR(1, T_t4_5, f_4_5, 1);
    mdot_4_5=mdot_0/(1+alfa)*((1-beta-epsilon1-epsilon2)*(1+f)+epsilon1+epsilon2);
    A_4_5=mdot_4_5*sqrt(T_t4_5)/((P_0*pi_r*pi_d*pi_cL*pi_cH*pi_b*pi_tH)*MFP_4_5);
             %A9 calculation
             
    mdot_9=mdot_4_5;
    A_9=mdot_9*sqrt(T_t9)/((P_0*pi_r*pi_d*pi_cL*pi_cH*pi_b*pi_tH*pi_tL)*MFP_9);
    ...
    S=f0/Fovermdot*3600;
    eta_P=(2*1*M_0*(1+alfa)*Fovermdot/a_0)/((1+f0*(1+alfa)-beta)*(V_9/a_0)^2+alfa*(V_19/a_0)^2-(1+alfa)*M_0^2);
    eta_TH = ((1 + f0 * (1 + alfa) - beta) * V_9^2 + alfa * V_19^2 - (V_0^2 + (C_TOL + C_TOH) * (h_0*1000))) / (2 * 1 * f0 * (h_PR*1000));
